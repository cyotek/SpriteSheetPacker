...
namespace Cyotek.Spriter.QuickPack.Packing
{
  ...
  ///   </para>
  /// </remarks>
  internal sealed class ArevaloRectanglePacker : RectanglePacker
  {
    ...
    private int _actualPackingAreaHeight;

    /// <summary>Current width of the packing area</summary>
    private int _actualPackingAreaWidth;
    ...
    {
      _packedRectangles = new List<Rectangle>();
      _anchors = new List<Point>();
      _anchors.Add(new Point(0, 0));
      _actualPackingAreaWidth = 1;
      _actualPackingAreaHeight = 1;
    }
    ...
    {
      _anchors.Add(new Point(rectangle.Left, rectangle.Top));
      _anchors.Add(new Point(rectangle.Right, rectangle.Bottom));
      _packedRectangles.Add(rectangle);
    }
    ...
    public override bool TryPack(int rectangleWidth, int rectangleHeight, out Point placement)
    {
      ...
      int anchorIndex = this.SelectAnchorRecursive(rectangleWidth, rectangleHeight, _actualPackingAreaWidth, _actualPackingAreaHeight);
      ...
      placement = _anchors[anchorIndex];
      ...
      // positions of the new rectangle
      {
        ...
        bool blocksAnchor = ((placement.X + rectangleWidth) > _anchors[anchorIndex].X) && ((placement.Y + rectangleHeight) > _anchors[anchorIndex].Y);
        if (blocksAnchor)
      ...
      _packedRectangles.Add(new Rectangle(placement.X, placement.Y, rectangleWidth, rectangleHeight));
      return true;
    }
    ...
    private int FindFirstFreeAnchor(int rectangleWidth, int rectangleHeight, int testedPackingAreaWidth, int testedPackingAreaHeight)
    {
      ...
      for (int index = 0; index < _anchors.Count; ++index)
      {
        ...
        if (this.IsFree(ref potentialLocation, testedPackingAreaWidth, testedPackingAreaHeight))
          return index;
      }
    ...
    private bool IsFree(ref Rectangle rectangle, int testedPackingAreaWidth, int testedPackingAreaHeight)
    {
      ...
      for (int index = 0; index < _packedRectangles.Count; ++index)
      {
        if (_packedRectangles[index].IntersectsWith(rectangle))
          return false;
      }
    ...
    private void OptimizePlacement(ref Point placement, int rectangleWidth, int rectangleHeight)
    {
      ...
      while (this.IsFree(ref rectangle, PackingAreaWidth, PackingAreaHeight))
      {
    ...
    private int SelectAnchorRecursive(int rectangleWidth, int rectangleHeight, int testedPackingAreaWidth, int testedPackingAreaHeight)
    {
      ...
      int freeAnchorIndex = this.FindFirstFreeAnchor(rectangleWidth, rectangleHeight, testedPackingAreaWidth, testedPackingAreaHeight);
      ...
      if (canEnlargeHeight && shouldEnlargeHeight)
      {
        ...
        return this.SelectAnchorRecursive(rectangleWidth, rectangleHeight, testedPackingAreaWidth, Math.Min(testedPackingAreaHeight * 2, PackingAreaHeight));
      }
      if (canEnlargeWidth)
      {
        // Try to double the width of the packing area
        return this.SelectAnchorRecursive(rectangleWidth, rectangleHeight, Math.Min(testedPackingAreaWidth * 2, PackingAreaWidth), testedPackingAreaHeight);
      }
