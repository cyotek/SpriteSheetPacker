...
namespace Cyotek.Spriter.QuickPack.Packing
{
  ...
  ///   </para>
  /// </remarks>
  internal sealed class CygonRectanglePacker : RectanglePacker
  {
    ...
    {
      this._heightSlices = new List<Point>();
      // At the beginning, the packing area is a single slice of height 0
      this._heightSlices.Add(new Point(0, 0));
    }
    ...
    public override void AddRectangle(Rectangle rectangle)
    {
      _heightSlices.Add(new Point(rectangle.Right, rectangle.Bottom));
    }
    ...
    public override bool TryPack(int rectangleWidth, int rectangleHeight, out Point placement)
    {
      ...
      bool fits = this.TryFindBestPlacement(rectangleWidth, rectangleHeight, out placement);
    ...
    private void IntegrateRectangle(int left, int width, int bottom)
    {
      ...
      int startSlice = this._heightSlices.BinarySearch(new Point(left, 0), SliceStartComparer.Default);
      ...
      firstSliceOriginalHeight = this._heightSlices[startSlice].Y;
      this._heightSlices[startSlice] = new Point(left, bottom);
      ...
      if (startSlice >= this._heightSlices.Count)
      {
        ...
        if (right < this.PackingAreaWidth)
          this._heightSlices.Add(new Point(right, firstSliceOriginalHeight));
      }
      else
      {
        // The rectangle doesn't start on the last slice
        int endSlice = this._heightSlices.BinarySearch(startSlice, this._heightSlices.Count - startSlice, new Point(right, 0), SliceStartComparer.Default);
        // Another direct hit on the final slice's end?
        if (endSlice > 0)
          this._heightSlices.RemoveRange(startSlice, endSlice - startSlice);
        else
        {
          ...
          // the rectangle
          int returnHeight;
          returnHeight = endSlice == startSlice ? firstSliceOriginalHeight : this._heightSlices[endSlice - 1].Y;
          ...
          this._heightSlices.RemoveRange(startSlice, endSlice - startSlice);
          if (right < this.PackingAreaWidth)
            this._heightSlices.Insert(startSlice, new Point(right, returnHeight));
        } // if endSlice > 0
    ...
    private bool TryFindBestPlacement(int rectangleWidth, int rectangleHeight, out Point placement)
    {
      ...
      int bestScore = this.PackingAreaHeight; // lower == better!
      ...
      int rightSliceIndex = this._heightSlices.BinarySearch(new Point(rectangleWidth, 0), SliceStartComparer.Default);
      if (rightSliceIndex < 0)
        rightSliceIndex = ~rightSliceIndex;
      while (rightSliceIndex <= this._heightSlices.Count)
      {
        ...
        int highest = this._heightSlices[leftSliceIndex].Y;
        for (int index = leftSliceIndex + 1; index < rightSliceIndex; ++index)
        {
          if (this._heightSlices[index].Y > highest)
            highest = this._heightSlices[index].Y;
        }
        // Only process this position if it doesn't leave the packing area
        if ((highest + rectangleHeight <= this.PackingAreaHeight))
        {
          int score = highest;
          if (score < bestScore)
          {
        ...
        ++leftSliceIndex;
        if (leftSliceIndex >= this._heightSlices.Count)
          break;
        // Advance the ending slice until we're on the proper slice again, given the new
        // starting position of the rectangle.
        int rightRectangleEnd = this._heightSlices[leftSliceIndex].X + rectangleWidth;
        for (; rightSliceIndex <= this._heightSlices.Count; ++rightSliceIndex)
        {
          int rightSliceStart;
          rightSliceStart = rightSliceIndex == this._heightSlices.Count ? this.PackingAreaWidth : this._heightSlices[rightSliceIndex].X;
        ...
        if (rightSliceIndex > this._heightSlices.Count)
          break;
      ...
      placement = new Point(this._heightSlices[bestSliceIndex].X, bestSliceY);
      return true;
    }
