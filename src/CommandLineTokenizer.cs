...
namespace Cyotek.Spriter.QuickPack
{
  internal sealed class CommandLineTokenizer
  {
    ...
    public T GetEnum<T>(string name, T defaultValue) where T : struct
    {
      if (!typeof(T).IsEnum)
      {
        throw new ArgumentException("Type is not an enum", "defaultValue");
      }
    ...
    internal bool GetBooleanValue(string value)
    {
      ...
      return !string.IsNullOrEmpty(value) && (value.Equals("true", StringComparison.InvariantCultureIgnoreCase) || int.TryParse(value, out integerValue) && integerValue != 0);
    }
